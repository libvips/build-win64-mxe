...
plugin_dirs+=" plugins/llvm-mingw"
...
if [ "$arch" = "x86_64" ] && [ -x "$(command -v wine64)" ]; then
  GIT_COMMIT="$git_commit" . packaging/variables.sh

  echo -n "testing packaging/$repackage_dir/bin/vips.exe ... "
  WINEDEBUG=fixme-all wine64 packaging/$repackage_dir/bin/vips.exe --help > /dev/null
  if [ $? -eq 0 ]; then
    echo "OK"
  else
